...
3. The change in x-direction is estimated. It is assumed that the car will return to the track centerline in 20.0 m along a straight line. The `x_point` values are set so that the quotient of the distance along the hypotenuse and the time step, `dt` yields the reference velocity, `ref_vel`.
